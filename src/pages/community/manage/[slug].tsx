...
function ManageCommunitySlug() {
  ...
  const [queuedMedia, handlers] = useListState<Media>([]);
      ...
      if (communitySlug) {
        ...
        if (res.ok) {
          ...
          const media = data.medias
            .filter((m: Media) => !m.watched)
            .map((m: Media, index: number) => ({ ...m, queue: index }));
  ...
  const items = queuedMedia.map((item: Media, index) => (
    <Draggable key={item.id} index={index} draggableId={item.id}>
      {(provided, snapshot) => (
        ...
        >
          <Text className={classes.position}>{(item.queue || 0) + 1}</Text>
          <div>
    ...
    return (
      <DragDropContext
        onDragEnd={(data) => {
          ...
          handlers.setItemProp(source.index || 0, "queue", source.index);
      ...
      {!isLoading && (
        <Container size="xs">
          ...
          <Group position="right">
            <Button loading={savingQueue} onClick={saveQueue}>Save Queue</Button>
          </Group>
        </Container>
      )}
    </Container>
